...
export function QuestionListItem({
  ...
  return (
    <List.Item>
      ...
      />
      <Space size="middle">
        <Link to={`/question/${is_teacher ? "teacher" : "student"}/${data.id}`}>
          查看
        </Link>
      </Space>
    </List.Item>
  ...
  return (
    <>
      ...
      <List
        ...
        renderItem={(item) => (
          <QuestionListItem data={item} is_teacher={is_teacher} />
        )}
      />
    </>
  );
...
export function QuestionShowcase({ question_id }: { question_id: number }) {
  ...
  return (
    <>
      ...
      <Button type={"primary"} onClick={submitQuestion} disabled={submitted}>
        提交
      </Button>
    </>
  );
