...
export function SpeechListItemD({
  data,
  renderLink,
}: {
  data: speech_data;
...
export function SpeechListItemN({
  data,
  is_teacher,
}: {
  data: speech_data;
  ...
  return (
    <List.Item>
      ...
      />
      <Space size={"middle"}>
        <Link to={`/speech/${is_teacher ? "teacher" : "student"}/${data.id}`}>
          查看
        </Link>
      </Space>
    </List.Item>
...
export function SpeechList({
  ...
  return (
    <>
      {contextHolder}
      <Typography.Title level={2}>演讲列表：</Typography.Title>
      {is_organizer && (
